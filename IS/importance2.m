...
while i < maxIter
    ...
    Y = modelFunc(X');
    wDen = [normpdf(X(1),parameterQ(1,1),parameterQ(1,2)), normpdf(X(2),parameterQ(2,1),parameterQ(2,2)), normpdf(X(3),parameterQ(3,1),parameterQ(3,2)) ];
    wNum = [normpdf(X(1),parameter(1,1),parameter(1,2)), normpdf(X(2),parameter(2,1),parameter(2,2)), normpdf(X(3),parameter(3,1),parameter(3,2))];
    w = wNum/wDen;
    if Y <= 0;
        wY = [wY;1*w];
    else
